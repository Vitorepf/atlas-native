    ...
    private var loading: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            Text("Índice, execução e capacidades chegam por contratos independentes.")
                .font(AtlasFont.mono(11))
                // Soft gold-quiet preparing caption.
                .foregroundStyle(AtlasTheme.accent.opacity(0.62))
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(.vertical, 24)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !subtitle.isEmpty {
                Text(subtitle)
                    ...
                    // Soft gold-quiet live subtitle meta.
                    .foregroundStyle(AtlasTheme.accent.opacity(0.62))
            }
        }
        .accessibilityElement(children: .combine)
...
struct ArenaPremiumQueuedView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            Text(model.arenaLiveEngineTitle)
                ...
                // Soft gold-quiet queued engine title meta.
    ...
    private func queuedMetric(_ value: String, _ label: String) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            // Soft gold-quiet queued metric caption.
            Text(label).font(AtlasFont.mono(10)).foregroundStyle(AtlasTheme.accent.opacity(0.62))
        }
        .accessibilityElement(children: .combine)
            ...
            if let progress = model.livePresentation?.progress {
                HStack(alignment: .lastTextBaseline, spacing: 7) {
                    ...
                    Text("de \(progress.total) casos confirmados")
                        .font(AtlasFont.mono(11))
                        // Soft gold-quiet terminal progress caption.
                        .foregroundStyle(AtlasTheme.accent.opacity(0.62))
                }
                .foregroundStyle(AtlasTheme.textPrimary)
                .accessibilityElement(children: .combine)
...
struct ArenaPremiumResultsView: View {
        ...
        return VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .lastTextBaseline) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("Índice").font(AtlasFont.mono(11)).foregroundStyle(AtlasTheme.accent.opacity(0.62))
                    HStack(alignment: .lastTextBaseline, spacing: 4) {
                        Text(ArenaFormat.score(engine.composite))
                            .font(AtlasFont.serif(62))
                        Text("/10")
                            .font(AtlasFont.mono(13, .medium))
                            // Soft gold-quiet index scale unit.
                            .foregroundStyle(AtlasTheme.accent.opacity(0.55))
                    }
                    .foregroundStyle(AtlasTheme.textPrimary)
            ...
            Text("Cobertura \(model.arenaCoverageText) · \(model.report?.claimAllowed == true ? "resultado final" : "resultado parcial")")
                .font(AtlasFont.mono(10))
                // Soft gold-quiet coverage honesty.
                .foregroundStyle(AtlasTheme.accent.opacity(0.62))
        }
    }
    ...
    private func smallMetric(
        ...
        VStack(alignment: .leading, spacing: 4) {
            // Soft gold-quiet small-metric label.
            Text(label).font(AtlasFont.mono(10)).foregroundStyle(AtlasTheme.accent.opacity(0.62))
    ...
    private func metric(_ value: String, _ label: String, gold: Bool) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            Text(label)
                ...
                // Soft gold-quiet pair metric label.
                .foregroundStyle(AtlasTheme.accent.opacity(0.62))
                .accessibilityHidden(true)
            Text(value)
                .font(AtlasFont.serif(28))
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Ordem, estado e progresso confirmados pelo servidor.")
                .font(AtlasFont.mono(12))
                // Soft gold-quiet execution honesty caption.
                .foregroundStyle(AtlasTheme.accent.opacity(0.62))
        }
        .accessibilityElement(children: .combine)
    ...
    private func runRow(_ run: AtlasArenaLiveRun) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(rowDetail(run))
                    .font(AtlasFont.mono(11))
                    // Soft gold-quiet run row detail.
                    .foregroundStyle(AtlasTheme.accent.opacity(0.62))
            }
            Spacer(minLength: 8)
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let arm = run.arm?.labelPT {
                Text(arm)
                    .font(AtlasFont.mono(12))
                    // Soft gold-quiet arm meta under suite title.
                    .foregroundStyle(AtlasTheme.accent.opacity(0.62))
            }
        }
    }
        ...
        if let done = run.casesDone, let total = run.casesTotal, total > 0, let fraction {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text(summaryLine(done: min(done, total), total: total))
                    .font(AtlasFont.mono(11))
                    // Soft gold-quiet progress summary.
                    .foregroundStyle(AtlasTheme.accent.opacity(0.62))
            }
        } else {
...
struct ArenaPremiumCapabilitiesView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .lastTextBaseline, spacing: 4) {
                ...
                Text("Capacidades cobertas")
                    .font(AtlasFont.mono(10))
                    // Soft gold-quiet coverage caption.
                    .foregroundStyle(AtlasTheme.accent.opacity(0.62))
                    .padding(.leading, 6)
            }
            .foregroundStyle(AtlasTheme.textPrimary)
    ...
    private var rows: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text(capabilitiesCaption)
                .font(AtlasFont.mono(10))
                // Soft gold-quiet capabilities footnote.
                .foregroundStyle(AtlasTheme.accent.opacity(0.55))
                .padding(.top, 16)
        }
    }
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            ...
                            if let caption = shortConfidence(capability) {
                                Text(caption)
                                    ...
                                    // Soft gold-quiet confidence caption.
                                    .foregroundStyle(AtlasTheme.accent.opacity(0.55))
                                    .lineLimit(1)
                                    .minimumScaleFactor(0.8)
                            }
    ...
    private func summaryMetric(_ value: Int, _ label: String, _ tone: ArenaPremiumTone) -> some View {
        HStack(spacing: 6) {
            ...
            Text(label).font(AtlasFont.mono(11)).foregroundStyle(AtlasTheme.accent.opacity(0.62))
        }
    }
...
struct ArenaPremiumCapabilityDetail: View {
    ...
    private var deltaColor: Color {
        ...
        if abs(delta) <= 0.005 { return AtlasTheme.accent.opacity(0.62) }
...
extension ArenaRunSheet {
    var submitButtonLabel: some View {
        Text("Rodar medição")
            ...
            .overlay(Capsule().stroke(input.isLocallyValidForSubmission ? AtlasTheme.goldBorder : AtlasTheme.goldBorder.opacity(0.4), lineWidth: 1))
            .atlasElevation(
                radius: 10,
                y: 3,
