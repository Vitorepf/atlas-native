...
- [ABERTO · M61/A12 · GPT-5.5→Codex/Server] Worker de medição da Arena para runs iniciados pelo app — A12 provou `POST /api/arena/runs` real pelo app e aparição em `runs/live`, mas o recibo declara `worker_implemented=false`; falta drenar runs queued → running/done → scoreboard/composto sem simular progresso. **2026-07-17 · Grok 4.5:** neste workspace `atlas-server` **ausente** → leafs A1.2b **BLOCKED(server)**; native segue A1.2a/d (recibo honesto) + A1.3 deep links. **2026-07-17 · Fable (ordem do operador "nunca use mock"):** mockllm/harness-only BANIDOS de payload público e da fila (server `55886a9`) — o worker deve drenar apenas motores reais; a prova de drenagem não pode mais usar mockllm.
- [ABERTO · Arena-goal · Fable→Codex/Server] Origem do run na Arena (goal 2 do operador 2026-07-17: "ver de onde começou, Mac ou iPhone") — `POST /api/arena/runs` deve aceitar `origin` opcional (`iphone|mac|cli`), persistir na fila e projetar em `runs_live.v1`; Core `AtlasArenaLiveRun.origin` opcional fail-open; casca então mostra badge de origem no AGORA. Sem o campo, a casca não inventa origem.
- [ABERTO · Arena-goal · Fable→Codex/Server] Catálogo de motores rodáveis — o run sheet lista só motores JÁ medidos (composite/scoreboard); para rodar um motor novo pela primeira vez falta `GET /api/arena/engines` (ModelRegistry.enabled(), sem harness_only) com nome público. Goal 1 pleno depende disso.
