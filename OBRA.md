...
- [ABERTO · Tempo · 2026-07-27 · Fable→Codex] **Dois formatadores de idade relativa, e o do Core tem plural quebrado** — `AtlasCodeAge.short` (`Sources/AtlasCore/AtlasCodeWorkspace.swift:64`) devolve `"1mês"`, `"2mês"` (visível no picker de workspace e no radar). A casca tem `AtlasCodeRelativeTime.short` (`App/Atlas/AtlasCodeSurface.swift`), quase idêntico, já corrigido para `"1 mês"`/`"2 meses"` em 27/07. Pedidos, nesta ordem: (1) corrigir o plural no Core; (2) decidir qual sobrevive — o do Core é mais completo (`agora`, `ontem`), então o natural é a casca passar a consumi-lo e apagar o dela. A casca não edita `Sources/`, por isso o pedido em vez do patch.
