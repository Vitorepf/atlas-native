...
- 2026-07-17 · Fable · **polish(ui) — deepen PlanCard steps residual honesty** · `cc35e42` · CICLO C cena 02 passos: peel `PlanCard+StepRow` (65) — spine decorativa silenciada; spoken `passo N de M` só com `plan.steps.count` real; trait `.isSelected` no passo em curso; breath RM no ponto atual; A11yID `plan-steps`/`plan-step-*`/`plan-progress`. `PlanCard+A11y` (58) — silêncio honesto sem checkpoint; spoken detail/chips/auditoria. Header (52) — título `.isHeader`; auditoria spoken composto; decorativos silenciados. Shell 83≤100. Zero Route. Prova: `wc -l` shell 83 Steps 61 StepRow 65 Header 52 A11y 58; `rg Aprovar App/Atlas`=0; Swift toolchain ausente neste cloud Linux — re-rodar `AtlasCoreChecks` + `make build` no Mac; device-pending.
