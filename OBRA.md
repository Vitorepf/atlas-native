...
| E-A4 | **PARCIAL** | **Grok 4.5** | deepen + fidelity matrix | E0 | Onda A4 casca deepen | A4.5 matrix DONE; A4.1 Session Hub DONE; Search/Workspace A11y+empty/offline deepen DONE; demais cenas ainda pendente | LiveNow hub + Search/Workspace polish `a037aa7` |
| E-A5 | **IN_PROGRESS** | **Grok 4.5** | `App/Widgets/*` + `AtlasDeepLink` | E-A1 deep links | Onda A5 fora-do-app | M84 Semana + lock accessories; `atlas://arena`→`.arena`; lock `queueLabel` cápsula gold no título; Island SD-2 ainda parcial; widgetURL Arena home entry opcional depois | `e0bce46` (arena deep link + lock queue capsule) + `183f01f`+`08f0dda`+`0ab1682`+`a702926` |
| E-A6 | **PENDING** | **operador** | device unlock + prints | passcode | Onda A6 DEVICE_PROVEN | U1–U10 + Arena E2E + M03/M04 | — |
| E-B | **PARCIAL** | **Grok 4.5** | AutonomosView + ConversationView + ConversationModel + RootChrome + ChangeReview/Radar + ConversationChrome/ExecutionStateCard peel + AtlasClient domain split + ArtifactSheet/TurnPresence peel + RootHomeSections + AtlasCodeGraphChrome + LiveTimeline+Filters + ChangeReview Diff/Council + ArenaSuiteSheet + InteractionRun+Polling + AtlasAutonomosTransfer + RichInputTypes + Attachments/Ledger/ArenaClient | E-A* | CICLO B compressão 1 | **Ledger (wc -l):** ConversationView~262; AutonomosView~297; Model 647→501 (+Attachments 156, meta <550); Chrome~181; Client~473; AtlasAutonomos 436→321 (+Ledger 116 backlog/delivered); RichInputContract 551→253 (+Types 306); InteractionRun 461→351 (+Polling 116); RootView 436→219 (+RootHomeSections 232); AtlasCodeView+Graph 399→191 (+AtlasCodeGraphChrome 215); LiveTimeline 289→235 (+Filters 62); ChangeReviewView~95; ChangeReviewSections 448→225 (+Diff 123 / Council 111); ArenaSuitesSection 234→120 (+SuiteSheet 125); Radar~66; ArtifactSheet~184; TurnPresence~249; Composer~196+Toolbar132+Messages138; PlanCard~227; Arena 369→245 DTOs (+AtlasClient+Arena 127 start/client); AgentActivity 364. **Ainda >400 sem ADR:** AtlasClient 473 (Model 501 ok sob meta 550/800). Status **PARCIAL** até zero monolito >400 sem ADR §6/§7. | peels + `wc -l`; build Mac-pending |
| E-C | **IN_PROGRESS** | **Grok 4.5** | Autonomos fleet silence + Conversation loadFailureKind + Nightly RM | E-B PARCIAL | CICLO C patamares | Frota/fila quietas sem incidente; load falha distingue kind; nightly dismiss=silêncio + Reduce Motion; `rg Aprovar App/Atlas`=0 | `dc787be` + este commit |
