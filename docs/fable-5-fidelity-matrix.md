...
| **04 · Aguardando sistema externo** | Espera nomeada ≠ pensamento; “pode sair”; lock avisa | `AtlasExecutionPresentationState.awaitingExternal` (+ `deadline` opcional) | `ExecutionStateCard`; fase em `TurnPresence` / Live Activity | device-pending | **PARCIAL** — peel `+AwaitingFailed`: deadline só publicado; «pode sair» só com `timer.paused` sem duplicar servidor (`b41d880`); receipts externos ricos + auto-advance ainda server/device-pending |
