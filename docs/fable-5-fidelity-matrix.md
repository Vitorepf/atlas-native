...
| **08 · Continuidade entre devices** | Handoff iPhone↔Mac/Terminal; mesma thread; zero prompt duplicado | `ConversationModel+Continuity.handoffToSurface`; `AtlasAiSurfaceDestination`; recibo `latestSurfaceHandoff` | Menu continuidade + `handoffReceipt` em `ConversationView`; LiveNow badge remoto (`sessions/live`) | device-pending | Recibo visual M14 + API handoff existem; theater multi-device completo e deep-link Continuity ainda rasos; **não** vira Session Hub route |
...
| **Fleet · Command Center** | Frota viva: uptimes, heartbeats, lease/atenção, transferir c/ prova, digest 14:00 | `AtlasAutonomos*` via `AutonomosModel` (fleet, digest, transfer, history, live) | `AutonomosView` (+ sheets); widgets frota leem `AtlasNativeSnapshot.fleet` | sim parcial Autônomos/Nightly; device-pending; heal→merge M01 BLOCKED(server) | Superfície própria OK; fidelidade Fable (uptime vivo, transfer UI, silêncio saudável) = A4.2; split monólito pendente B1 |
| **Island · Dynamic Island / LA** | Mesma ilha morph: executando → plano → resposta → atenção | `AtlasTurnAttributes` + `TurnPresence` ContentState; timer de `AtlasExecutionPresentationState.Timer`; snapshot App Group | `TurnPresence`; `AtlasActivityAttributes`; `App/Widgets/*` | U10 instalado (`c3eda84`); falta print DEVICE_PROVEN; SD-2 estados incompletos | Ciclo parcial (fase/timer/finished); A5.4 SD-2 (ATT/EXT/FAIL/N/M…) e LA Arena/Fleet ainda gaps; App Intents mecânicos §5 M89 |
